...
if (!(Test-Path -LiteralPath $PackagePath)) {
  throw "PackagePath nao encontrado: $PackagePath"
}
...
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PackagePath "install.ps1") -InstallDir $root -NoService -SkipHealthcheck
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root "start.ps1")
& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root "healthcheck.ps1")
