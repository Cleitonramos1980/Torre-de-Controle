...
export const SESMT_FORM_SCHEMAS = {
    "atividades-por-nr": [
...
function asHttpError(message, statusCode = 400) {
    return Object.assign(new Error(message), { statusCode });
}
export function getSesmtFormSchema(moduleKey) {
    return SESMT_FORM_SCHEMAS[moduleKey] ?? [];
}
export function validateSpecificFieldKey(moduleKey, fieldKey) {
...
export function validateSesmtSpecificData(moduleKey, payloadValue, priorValue) {
    const schema = getSesmtFormSchema(moduleKey);
    if (schema.length === 0)
        return undefined;
    const payload = payloadValue == null
        ? {}
        : (typeof payloadValue === "object" && !Array.isArray(payloadValue) ? payloadValue : null);
    if (payload === null) {
