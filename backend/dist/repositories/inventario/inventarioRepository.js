...
import { syncInventarioLojasFromPcfilial } from "./initTables.js";
...
function toOracleDateLiteral(input) {
...
function calculateRiskLevel(acuracidade) {
    if (acuracidade <= 0)
        return "sem_contagem";
    if (acuracidade < 80)
        return "alta";
...
function deriveConsensusStatus(qtd1, qtd2, qtd3) {
    const vals = [qtd1, qtd2, qtd3].filter((value) => value != null);
    if (vals.length < 2) {
        return { status: "PENDENTE", consenso: null, escalado: false };
    }
    const [a, b, c] = [qtd1, qtd2, qtd3];
    if (a != null && b != null && a === b)
        return { status: "CONSENSO_2_DE_3", consenso: a, escalado: false };
    if (a != null && c != null && a === c)
        return { status: "CONSENSO_2_DE_3", consenso: a, escalado: false };
    if (b != null && c != null && b === c)
        return { status: "CONSENSO_2_DE_3", consenso: b, escalado: false };
...
async function writeAuditEvent(input) {
    appendAudit(input.acao, input.entidade, input.entidadeId ?? "N/A", input.detalhe ?? "", input.usuarioNome ?? "system");
    if (!isOracleEnabled())
        return;
...
async function listContagemItemsOracle(contagemId) {
    ...
    ORDER BY ORDEM_ITEM, ID`, { contagemId });
    return rows.map((row) => ({
        id: String(row.ID),
...
function listContagemItemsFallback(contagem) {
...
function mapContagemRow(row, itens) {
...
async function loadContagemOracle(id) {
...
function recalculateContagemFromItens(itens) {
...
function calculateChecklistSummary(items, bloqueioNc) {
    if (items.length === 0)
        return { statusGeral: "ABERTO", progresso: 0 };
    const allPendente = items.every((item) => item.status === "PENDENTE");
    if (allPendente)
        return { statusGeral: "ABERTO", progresso: 0 };
    const concluiveis = new Set(["CONCLUIDO", "CANCELADO", "NAO_APLICAVEL"]);
...
function ensureFallbackChecklists() {
    if (!Array.isArray(db.inventarioChecklists)) {
        db.inventarioChecklists = [];
    }
    const list = db.inventarioChecklists;
    if (list.length > 0)
        return list;
    const seeds = [
...
async function loadChecklistByIdOracle(checklistId) {
    const checklist = await queryOne(`SELECT
      ID,
      NOME,
      UNIDADE,
    ...
    WHERE ID = :id`, { id: checklistId });
    if (!checklist)
        return null;
    const blocoRows = await queryRows(`SELECT ID, CODIGO, ORDEM, NOME, PROGRESSO, TOTAL_ITENS
     FROM CHECKLIST_BLOCOS
     WHERE CHECKLIST_ID = :checklistId
     ORDER BY ORDEM, ID`, { checklistId });
    const itemRows = await queryRows(`SELECT
      ID,
      BLOCO_ID,
      CODIGO,
      ORDEM,
     ...
     WHERE CHECKLIST_ID = :checklistId
     ORDER BY BLOCO_ID, ORDEM, ID`, { checklistId });
    const evidRows = await queryRows(`SELECT
      ID,
      ITEM_ID,
      NOME_ARQUIVO,
      CAMINHO,
     ...
     WHERE CHECKLIST_ID = :checklistId
     ORDER BY CRIADO_EM DESC`, { checklistId });
    const histRows = await queryRows(`SELECT
      ID,
      ITEM_ID,
      ACAO,
      DETALHE,
     ...
     WHERE CHECKLIST_ID = :checklistId
     ORDER BY DATA_EVENTO DESC`, { checklistId });
    const evidByItem = new Map();
    ...
    return {
        ...
        statusGeral: String(checklist.STATUS_GERAL),
...
async function recalcChecklistStatusOracle(checklistId) {
    const rows = await queryRows(`SELECT STATUS, NC_ABERTA
     FROM CHECKLIST_ITENS
     WHERE CHECKLIST_ID = :checklistId`, { checklistId });
    const checklist = await queryOne(`SELECT BLOQUEAR_CONCLUSAO_POR_NC FROM CHECKLISTS_PRE_INVENTARIO WHERE ID = :id`, { id: checklistId });
    const bloqueioNc = Number(checklist?.BLOQUEAR_CONCLUSAO_POR_NC ?? 1) === 1;
    const summary = calculateChecklistSummary(rows.map((row) => ({
        status: String(row.STATUS ?? "PENDENTE"),
        ncAberta: Number(row.NC_ABERTA ?? 0) === 1,
    })), bloqueioNc);
    await execDml(`UPDATE CHECKLISTS_PRE_INVENTARIO
        SET STATUS_GERAL = :statusGeral,
            PROGRESSO_GERAL = :progresso,
            UPDATED_AT = SYSTIMESTAMP
      WHERE ID = :id`, { id: checklistId, statusGeral: summary.statusGeral, progresso: summary.progresso });
    const blocoRows = await queryRows(`SELECT ID FROM CHECKLIST_BLOCOS WHERE CHECKLIST_ID = :checklistId`, { checklistId });
    for (const bloco of blocoRows) {
        const blocoId = String(bloco.ID);
        const items = await queryRows(`SELECT STATUS FROM CHECKLIST_ITENS WHERE BLOCO_ID = :blocoId`, { blocoId });
        ...
        await execDml(`UPDATE CHECKLIST_BLOCOS
          SET PROGRESSO = :progresso,
              TOTAL_ITENS = :totalItens,
              UPDATED_AT = SYSTIMESTAMP
        WHERE ID = :id`, { id: blocoId, progresso, totalItens: items.length });
    }
...
export const inventarioRepository = {
    async listLojas() {
        if (!isOracleEnabled())
            return db.inventarioLojas;
        try {
            await syncInventarioLojasFromPcfilial();
        }
        catch {
            // Mantem resposta da API mesmo se a sincronizacao do cadastro falhar.
        }
        const rows = await queryRows(`SELECT ID, CODIGO, NOME, REGIONAL, GERENTE, SUPERVISOR FROM LOJAS_INVENTARIO WHERE ATIVO = 1 ORDER BY CODIGO`);
        return rows.map((row) => ({
            id: String(row.ID),
    ...
    async listDepartamentos() {
        if (!isOracleEnabled())
            return db.inventarioDepartamentos;
        const rows = await queryRows(`SELECT ID, CODIGO, NOME FROM DEPARTAMENTOS_INVENTARIO WHERE ATIVO = 1 ORDER BY CODIGO`);
        return rows.map((row) => ({
            id: String(row.ID),
            codigo: String(row.CODIGO),
            nome: String(row.NOME),
    ...
    async listFrequencias() {
        if (!isOracleEnabled())
            return db.inventarioFrequencias;
        const rows = await queryRows(`SELECT
        f.ID,
        f.LOJA_ID,
        l.NOME AS LOJA_NOME,
    ...
    async listContagens() {
        if (!isOracleEnabled()) {
            return db.inventarioContagens.map((contagem) => ({
                ...contagem,
    ...
    async getContagemById(id) {
        if (!isOracleEnabled()) {
            const contagem = db.inventarioContagens.find((item) => item.id === id);
            if (!contagem)
                return null;
    ...
    async createContagem(input, actor) {
        if (!isOracleEnabled()) {
            const rec = {
                ...input,
        ...
        const status = String(input.status ?? "NAO_INICIADO");
        ...
        await execDml(`INSERT INTO CONTAGENS (
        ...
        for (const item of itens) {
            await execDml(`INSERT INTO ITENS_CONTAGEM (
    ...
    async updateContagem(id, input, actor) {
        if (!isOracleEnabled()) {
            const idx = db.inventarioContagens.findIndex((item) => item.id === id);
            if (idx < 0)
                return null;
            db.inventarioContagens[idx] = { ...db.inventarioContagens[idx], ...input };
            await writeAuditEvent({
                modulo: "INVENTARIO",
                acao: "ATUALIZAR",
                entidade: "CONTAGEM",
        ...
        await execDml(`UPDATE CONTAGENS
         SET STATUS = :status,
       ...
       WHERE ID = :id`, {
        ...
        if (Array.isArray(input.itens)) {
            for (const item of input.itens) {
                ...
                const consenso = deriveConsensusStatus(qtd1 == null ? null : asNumber(qtd1), qtd2 == null ? null : asNumber(qtd2), qtd3 == null ? null : asNumber(qtd3));
                ...
                await execDml(`UPDATE ITENS_CONTAGEM
        ...
        ESCALADO_SUPERVISOR
      FROM ITENS_CONTAGEM
      WHERE CONTAGEM_ID = :contagemId`, { contagemId: id });
    ...
    async validarContagem(id, validadoPor, actor) {
        const current = await this.getContagemById(id);
        if (!current)
            return null;
        const updated = await this.updateContagem(id, {
            status: "VALIDADO",
            validadoPor,
            validadoEm: nowIso(),
        }, actor);
        if (!isOracleEnabled()) {
            ...
            SET STATUS = 'VALIDADO',
                ...
                UPDATED_BY = :updatedBy,
                UPDATED_AT = SYSTIMESTAMP
          WHERE ID = :id`, { id, validadoPor, updatedBy: actor?.userName ?? "system" });
        }
        await writeAuditEvent({
            modulo: "INVENTARIO",
            acao: "VALIDAR",
    ...
    async solicitarRecontagem(id, solicitadoPor, actor) {
        const current = await this.getContagemById(id);
        if (!current)
            return null;
        const updated = await this.updateContagem(id, {
    ...
    async upsertDivergenciaDiaria(contagemId) {
        if (!isOracleEnabled())
            return;
        const contagem = await this.getContagemById(contagemId);
        if (!contagem)
            return;
        const existing = await queryOne(`SELECT ID FROM DIVERGENCIAS_DIARIAS WHERE CONTAGEM_ID = :contagemId`, { contagemId });
        const id = existing?.ID ? String(existing.ID) : randomUUID();
        const nivel = calculateRiskLevel(asNumber(contagem.acuracidade, 0));
        if (existing?.ID) {
            await execDml(`UPDATE DIVERGENCIAS_DIARIAS
        ...
        await execDml(`INSERT INTO DIVERGENCIAS_DIARIAS (
    ...
    async listDivergencias() {
        if (!isOracleEnabled())
            return db.inventarioDivergencias;
        const rows = await queryRows(`SELECT
      ...
      ORDER BY d.DATA_REFERENCIA DESC`);
        return rows.map((row) => ({
            ...
            status: String(row.STATUS_CONTAGEM),
    ...
    async getChecklistById(id) {
        if (!isOracleEnabled()) {
            ...
            return {
                ...checklist,
                blocos: checklist.blocos.map((bloco) => ({
                    ...bloco,
                    itens: bloco.itens.map((item) => mapChecklistItemFallback(item)),
                })),
            };
        }
        return loadChecklistByIdOracle(id);
    },
    async createChecklist(payload, actor) {
        ...
        for (const bloco of CHECKLIST_PRE_INVENTARIO_TEMPLATE) {
            ...
            await execDml(`INSERT INTO CHECKLIST_BLOCOS (ID, CHECKLIST_ID, CODIGO, ORDEM, NOME, PROGRESSO, TOTAL_ITENS)
    ...
    async updateChecklist(id, payload, actor) {
        if (!isOracleEnabled()) {
            const list = ensureFallbackChecklists();
    ...
    async deleteChecklist(id, actor) {
        if (!isOracleEnabled()) {
            const list = ensureFallbackChecklists();
            const idx = list.findIndex((item) => item.id === id);
            if (idx < 0)
        ...
        const exists = await queryOne(`SELECT ID FROM CHECKLISTS_PRE_INVENTARIO WHERE ID = :id`, { id });
        if (!exists)
            return false;
    ...
    async updateChecklistItem(checklistId, itemId, payload, actor) {
        if (!isOracleEnabled()) {
            const checklist = ensureFallbackChecklists().find((item) => item.id === checklistId);
            if (!checklist)
                return null;
            const item = checklist.blocos.flatMap((bloco) => bloco.itens).find((it) => it.id === itemId);
            if (!item)
                return null;
            const previous = { ...item };
            ...
            const summary = calculateChecklistSummary(checklist.blocos.flatMap((bloco) => bloco.itens).map((it) => ({ status: it.status, ncAberta: !!it.nc })), true);
       ...
       FROM CHECKLIST_ITENS
       WHERE CHECKLIST_ID = :checklistId
         AND ID = :itemId`, { checklistId, itemId });
        if (!current)
            return null;
        const merged = {
            ...
            criticidade: (payload.criticidade ?? current.CRITICIDADE),
        ...
        await execDml(`UPDATE CHECKLIST_ITENS
          SET STATUS = :status,
              RESPONSAVEL = :responsavel,
        ...
        await execDml(`INSERT INTO CHECKLIST_ITENS_HISTORICO (
    ...
    async listChecklistItemHistorico(checklistId, itemId) {
        if (!isOracleEnabled()) {
            const checklist = ensureFallbackChecklists().find((item) => item.id === checklistId);
            const item = checklist?.blocos.flatMap((bloco) => bloco.itens).find((it) => it.id === itemId);
            return item?.historico ?? [];
        }
        const rows = await queryRows(`SELECT ID, ACAO, DETALHE, USUARIO, DATA_EVENTO, VALOR_ANTERIOR, VALOR_NOVO
       FROM CHECKLIST_ITENS_HISTORICO
       WHERE CHECKLIST_ID = :checklistId
         AND ITEM_ID = :itemId
       ORDER BY DATA_EVENTO DESC`, { checklistId, itemId });
        return rows.map((row) => ({
            id: String(row.ID),
    ...
    async addChecklistItemEvidence(checklistId, itemId, payload, actor) {
        ...
        await execDml(`INSERT INTO SGQ_ANEXOS (
        ...
        await execDml(`INSERT INTO CHECKLIST_ITENS_EVIDENCIAS (
    ...
    async getChecklistDashboard() {
        const checklists = await this.listChecklists();
        const allItems = checklists.flatMap((checklist) => checklist.blocos.flatMap((bloco) => bloco.itens.map((item) => ({
            ...item,
