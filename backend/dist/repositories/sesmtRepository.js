...
import { getSesmtModuleDefinition, SESMT_EXECUTIVE_VIEWS, SESMT_MENU_TREE, SESMT_MODULE_DEFINITIONS, } from "./sesmtCatalog.js";
import { validateSesmtSpecificData, validateSpecificFieldKey, } from "./sesmtFormSchemaCatalog.js";
...
function ensureStore() {
    ...
    if (!Array.isArray(db.sesmt.registros))
        db.sesmt.registros = [];
    if (!Array.isArray(db.sesmt.acessosSensiveis))
        db.sesmt.acessosSensiveis = [];
    if (!Array.isArray(db.sesmt.favoritePresets))
        db.sesmt.favoritePresets = [];
    return db.sesmt;
}
function nowIso() {
...
function assertWrite(profile, isSensitive) {
...
function buildHistoryEntry(acao, descricao, usuario) {
    return {
...
function normalizeSpecificValue(value) {
    if (value === null)
        return null;
...
function normalizeSpecificData(payloadValue, priorValue) {
    const merged = { ...(priorValue || {}) };
...
function normalizeSpecificFilters(value) {
...
function applyUnitFilter(records, allowedUnits) {
    if (allowedUnits.length === 0)
        return [];
    return records.filter((record) => allowedUnits.includes(record.unidade));
}
function sortRecords(records, sortBy, sortDir = "desc") {
...
function mapRecordInput(moduleKey, payload, userName, existing) {
    ...
    return {
        ...
        tags: Array.isArray(payload.tags) ? payload.tags.map((tag) => String(tag).trim()).filter(Boolean) : prior.tags || [],
        anexos: prior.anexos || [],
        evidencias: prior.evidencias || [],
        historico: prior.historico || [],
        origemVinculada: Array.isArray(payload.origemVinculada)
            ? payload.origemVinculada.map((item) => ({ tipo: String(item.tipo || ""), id: String(item.id || "") }))
...
function withModuleMetadata(record) {
...
function normalizeCollaboratorStatus(value) {
...
function buildDossieAlerts(input) {
    const alertas = [];
    if (input.exames.some((row) => safeStatus(row.status, "") === "VENCIDO"))
        alertas.push("Exame vencido");
    if (input.exames.some((row) => safeStatus(row.status, "") === "PENDENTE"))
        alertas.push("Exame pendente");
    if (input.asos.some((row) => safeStatus(row.status, "") === "VENCIDO"))
        alertas.push("ASO vencido");
    if (input.asos.some((row) => safeStatus(row.status, "") === "PROXIMO_VENCIMENTO"))
        alertas.push("ASO próximo do vencimento");
    if (input.treinamentos.some((row) => safeStatus(row.status, "") === "VENCIDO"))
        alertas.push("Treinamento vencido");
    if (input.treinamentos.some((row) => safeStatus(row.status, "") === "PENDENTE"))
        alertas.push("Treinamento pendente");
    ...
    if (input.restricoes.some((row) => safeStatus(row.status, "") === "ATIVA"))
        alertas.push("Restrição ocupacional ativa");
...
async function appendSesmtAuditEvent(input) {
    ...
    if (!isOracleEnabled())
        return;
    try {
        await execDml(`INSERT INTO SESMT_LOG_AUDITORIA (
...
async function loadOracleDossie(actor, allowedUnits) {
    const base = await queryOne(`SELECT
      ID,
      MATRICULA,
      NOME,
      CPF,
    ...
    WHERE ID = :id`, { id: actor.colaboradorId });
    if (!base)
        return null;
    ...
    const examesRows = await queryRows(`SELECT
      ID,
      TIPO,
    ...
    ORDER BY DATA_EXAME DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const asosRows = await queryRows(`SELECT
      ID,
      TIPO,
    ...
    FROM SESMT_ASOS
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_ASO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const treinamentosRows = await queryRows(`SELECT
      ID,
      NOME,
      TIPO,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_TREINAMENTO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const atendimentosRows = await queryRows(`SELECT
      ID,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_ATENDIMENTO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const vacinasRows = await queryRows(`SELECT
      ID,
      VACINA,
      DOSE,
    ...
    FROM SESMT_VACINAS
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_APLICACAO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const medicacoesRows = await queryRows(`SELECT
      ID,
      MEDICAMENTO,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_REGISTRO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const acidentesRows = await queryRows(`SELECT
      ID,
      TIPO,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_OCORRENCIA DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const afastamentosRows = await queryRows(`SELECT
      ID,
      TIPO,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_INICIO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const atestadosRows = await queryRows(`SELECT
      ID,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_ATESTADO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const restricoesRows = await queryRows(`SELECT
      ID,
      DESCRICAO,
      TO_CHAR(DATA_INICIO, 'YYYY-MM-DD') AS DATA_INICIO,
      TO_CHAR(DATA_FIM, 'YYYY-MM-DD') AS DATA_FIM,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_INICIO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const episRows = await queryRows(`SELECT
      ID,
      DESCRICAO,
      CA,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_ENTREGA DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const comunicadosRows = await queryRows(`SELECT
      ID,
      TITULO,
      TIPO,
      TO_CHAR(DATA_REGISTRO, 'YYYY-MM-DD') AS DATA_REGISTRO,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_REGISTRO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    const documentosRows = await queryRows(`SELECT
      ID,
      NOME,
      TIPO,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_DOCUMENTO DESC NULLS LAST, CREATED_AT DESC`, { id: actor.colaboradorId });
    let laudosRows = await queryRows(`SELECT
      l.ID,
      l.TIPO,
    ...
    ORDER BY l.DATA_EMISSAO DESC NULLS LAST, l.CREATED_AT DESC`, { id: actor.colaboradorId });
    if (laudosRows.length === 0) {
        laudosRows = await queryRows(`SELECT
        ID,
        TIPO,
      ...
      ORDER BY DATA_EMISSAO DESC NULLS LAST, CREATED_AT DESC`, { unidade });
    }
    let timelineRows = await queryRows(`SELECT
      ID,
    ...
    WHERE COLABORADOR_ID = :id
    ORDER BY DATA_EVENTO DESC, CREATED_AT DESC`, { id: actor.colaboradorId });
...
function buildFallbackDossie(actor, allowedUnits) {
    const store = ensureStore();
...
async function resolveDossie(actor) {
...
export const sesmtRepo = {
    ...
    getLookups(profile, userId) {
        assertRead(profile);
        const store = ensureStore();
        const allowedUnits = getAllowedUnits(profile, userId);
        return {
            unidades: store.unidades.filter((unit) => allowedUnits.includes(unit.codigo)),
            setores: store.setores.filter((setor) => allowedUnits.includes(setor.unidade)),
            cargos: store.cargos,
            funcoes: store.funcoes,
            colaboradores: store.colaboradores.filter((col) => allowedUnits.includes(col.unidade)),
            profissionaisSesmt: store.profissionaisSesmt.filter((prof) => allowedUnits.includes(prof.unidade)),
    ...
    listRecords(input) {
        ...
        let records = store.registros.filter((record) => record.moduleKey === input.moduleKey);
        records = applyUnitFilter(records, allowedUnits);
        if (input.unidade)
            records = records.filter((record) => record.unidade === input.unidade);
        if (input.status)
        ...
        if (input.criticidade)
            records = records.filter((record) => record.criticidade === input.criticidade);
        if (input.nr)
            records = records.filter((record) => record.nr === input.nr);
        if (input.periodStart)
            records = records.filter((record) => !record.periodoInicio || record.periodoInicio >= input.periodStart);
        if (input.periodEnd)
            records = records.filter((record) => !record.periodoFim || record.periodoFim <= input.periodEnd);
    ...
    getRecordById(input) {
        ...
        const moduleDef = getSesmtModuleDefinition(input.moduleKey);
        if (!moduleDef)
            throw Object.assign(new Error("Submodulo nao encontrado."), { statusCode: 404 });
        if (moduleDef.visibility === "SENSITIVE_HEALTH") {
            assertSensitiveRead(input.profile, moduleDef.key, input.userName);
        }
        const store = ensureStore();
        const allowedUnits = getAllowedUnits(input.profile, input.userId);
        const record = store.registros.find((item) => item.moduleKey === input.moduleKey && item.id === input.id);
        if (!record)
            throw Object.assign(new Error("Registro nao encontrado."), { statusCode: 404 });
    ...
    createRecord(input) {
        ...
        store.registros.unshift(record);
        if (Array.isArray(store[moduleDef.collectionKey])) {
            store[moduleDef.collectionKey].unshift(record);
        }
    ...
    getFavoritePreset(input) {
        ...
        const store = ensureStore();
        const favorite = store.favoritePresets
    ...
    saveFavoritePreset(input) {
        ...
        const store = ensureStore();
        const list = store.favoritePresets;
    ...
    getMasterDashboard(profile, userId, userName, filters) {
        assertRead(profile);
        const store = ensureStore();
        const allowedUnits = getAllowedUnits(profile, userId);
        const filterUnit = cleanString(filters?.unidade);
        let records = applyUnitFilter(store.registros, allowedUnits);
        if (filterUnit)
            records = records.filter((record) => record.unidade === filterUnit);
        if (filters?.periodStart)
            records = records.filter((record) => !record.createdAt || record.createdAt.slice(0, 10) >= filters.periodStart);
        if (filters?.periodEnd)
            records = records.filter((record) => !record.createdAt || record.createdAt.slice(0, 10) <= filters.periodEnd);
        const now = new Date();
        ...
        const documentosVencendo = store.documentosControlados
            .filter((doc) => {
        ...
        const byUnit = records.reduce((acc, record) => {
            if (!acc[record.unidade])
                acc[record.unidade] = { total: 0, concluidos: 0, criticos: 0 };
            acc[record.unidade].total += 1;
            if (record.status === "CONCLUIDO")
                acc[record.unidade].concluidos += 1;
            if (record.criticidade === "CRITICA")
                acc[record.unidade].criticos += 1;
            return acc;
        }, {});
        ...
        const visaoNr = Object.entries(records.reduce((acc, record) => {
            const key = record.nr || "NR-NA";
            acc[key] = (acc[key] || 0) + 1;
            return acc;
        }, {})).map(([nr, total]) => ({ nr, total }));
        const custoAnual = store.custos
    ...
    getMaturityDashboard(profile, userId, userName, filters) {
    ...
    getPredictiveDashboard(profile, userId, userName, filters) {
        assertRead(profile);
        const base = sesmtRepo.getMasterDashboard(profile, userId, userName, filters);
    ...
    getIndicatorsDashboard(profile, userId, userName, filters) {
        assertRead(profile);
        const base = sesmtRepo.getMasterDashboard(profile, userId, userName, filters);
        const scopedRecords = applyUnitFilter(ensureStore().registros, getAllowedUnits(profile, userId));
        const statusDistribuicao = Object.entries(scopedRecords.reduce((acc, record) => {
            acc[record.status] = (acc[record.status] || 0) + 1;
            return acc;
        }, {})).map(([status, total]) => ({ status, total }));
        const criticidadeDistribuicao = Object.entries(scopedRecords.reduce((acc, record) => {
    ...
    getGerencialOcupacionalDashboard(profile, userId, userName, filters) {
        assertRead(profile);
        assertSensitiveRead(profile, "gerencial-ocupacional", userName);
        const store = ensureStore();
        const allowedUnits = getAllowedUnits(profile, userId);
        const selectedUnit = cleanString(filters?.unidade);
        const records = applyUnitFilter(store.registros, allowedUnits)
    ...
    async listDossieColaboradores(input) {
        ...
        ORDER BY NOME`);
            ...
            if (filterUnit)
                items = items.filter((item) => item.unidade === filterUnit);
            if (status)
                items = items.filter((item) => item.status === status);
            if (search) {
                items = items.filter((item) => [item.nome, item.matricula, item.cpf, item.cargo, item.unidade]
                    .filter(Boolean)
        ...
        const store = ensureStore();
        const base = Array.isArray(store.colaboradores) ? store.colaboradores : [];
