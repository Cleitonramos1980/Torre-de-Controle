...
import { asBool, asNumber, findSetorIdByNome, getAuditTimestampColumns, pickOptionalColumn, toIso, uid, } from "./shared.js";
let schemaPromise = null;
async function getChecklistSchema() {
    if (!schemaPromise) {
        schemaPromise = (async () => {
...
function columnOrDefault(columnName, alias, fallback) {
...
function mapModelFallback(input) {
    return {
        id: input.id,
        nome: input.nome,
        setor: input.setor,
...
async function mapModelFromOracle(row) {
    ...
    const items = await queryRows(`SELECT ID,
      ...
      ORDER BY ${orderByItems}`, { modeloId: row.ID });
    return {
        id: row.ID,
        nome: row.NOME,
        setor: row.SETOR,
...
export async function listModelos() {
    if (!isOracleEnabled()) {
        return db.inspecoesModelos.map((item) => mapModelFallback(item));
    }
    const schema = await getChecklistSchema();
    ...
    const updatedExpr = schema.modeloUpdatedAtColumn
        ...
        : createdExpr;
    const rows = await queryRows(`SELECT m.ID,
            m.NOME,
            m.DESCRICAO,
            s.NOME AS SETOR,
...
export async function getModeloById(id) {
    if (!isOracleEnabled()) {
        const found = db.inspecoesModelos.find((item) => item.id === id);
    ...
    const updatedExpr = schema.modeloUpdatedAtColumn
        ? `TO_CHAR(m.${schema.modeloUpdatedAtColumn}, 'YYYY-MM-DD"T"HH24:MI:SS')`
        : createdExpr;
    const row = await queryOne(`SELECT m.ID,
            m.NOME,
            m.DESCRICAO,
            s.NOME AS SETOR,
            ${schema.modeloAtivoColumn ? `m.${schema.modeloAtivoColumn} AS ATIVO` : "1 AS ATIVO"},
...
async function upsertModelItems(modeloId, setorId, items) {
    const schema = await getChecklistSchema();
    ...
    for (const [index, item] of items.entries()) {
        ...
        const existsRow = await queryOne(`SELECT COUNT(*) AS CNT FROM INS_MODELO_CHECKLIST_ITEM WHERE ID = :id`, { id: itemId });
        ...
        if (exists) {
            ...
            await execDml(`UPDATE INS_MODELO_CHECKLIST_ITEM
          ...
          WHERE ID = :id`, updateBinds);
            ...
            await execDml(`INSERT INTO INS_MODELO_CHECKLIST_ITEM (${insertColumns.join(", ")})
...
export async function createModelo(data) {
    ...
    if (!isOracleEnabled()) {
        ...
        db.inspecoesModelos.push(model);
        return model;
    }
    ...
    await execDml(`INSERT INTO INS_MODELO_CHECKLIST (${columns.join(", ")})
     VALUES (${values.join(", ")})`, {
        id,
        nome: data.nome,
    ...
    if (!created)
        throw new Error("Falha ao carregar modelo criado");
    return created;
}
export async function updateModelo(id, data) {
    if (!isOracleEnabled()) {
        const index = db.inspecoesModelos.findIndex((item) => item.id === id);
        if (index < 0)
            return null;
    ...
    if (!setorId)
        throw new Error(`Setor nao encontrado: ${setorName}`);
    const schema = await getChecklistSchema();
    const updateClauses = [
        "NOME = :nome",
        "DESCRICAO = :descricao",
    ...
    await execDml(`UPDATE INS_MODELO_CHECKLIST
        SET ${updateClauses.join(", ")}
      WHERE ID = :id`, {
        id,
