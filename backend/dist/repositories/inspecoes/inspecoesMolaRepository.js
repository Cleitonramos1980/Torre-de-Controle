...
import { asBool, asNumber, getAuditTimestampColumns, pickFirstExistingColumn, pickOptionalColumn, toIso, uid, } from "./shared.js";
let schemaPromise = null;
async function getMolaSchema() {
    if (!schemaPromise) {
        schemaPromise = (async () => {
...
function mapFallbackPadrao(item) {
    return {
        id: item.id,
...
function mapFallbackInspecao(item) {
    return {
        id: item.id,
...
export async function listPadroesMola() {
    if (!isOracleEnabled()) {
        return db.inspecoesPadroesMola.map((item) => mapFallbackPadrao(item));
    }
...
export async function createPadraoMola(data) {
    const id = data.id ?? uid("PM");
    if (!isOracleEnabled()) {
        const payload = mapFallbackPadrao({ ...data, id });
        db.inspecoesPadroesMola.push(payload);
        return payload;
    }
    ...
    await execDml(`INSERT INTO INS_MOLA_PADRAO (${columns.join(", ")})
     VALUES (${values.join(", ")})`, binds);
    return {
        id,
        alturaTipo: data.alturaTipo,
        item: data.item,
...
export async function updatePadraoMola(id, data) {
    if (!isOracleEnabled()) {
        const idx = db.inspecoesPadroesMola.findIndex((item) => item.id === id);
        if (idx < 0)
            return null;
    ...
    const current = await queryOne(`SELECT ID, ALTURA_TIPO, ITEM, DESCRICAO, PADRAO, MINIMO, MAXIMO, UNIDADE, ATIVO
       FROM INS_MOLA_PADRAO
      WHERE ID = :id`, { id });
    if (!current)
        return null;
    ...
    await execDml(`UPDATE INS_MOLA_PADRAO
...
async function loadMedicoes(inspecaoId) {
    const rows = await queryRows(`SELECT ID,
            PADRAO_ID,
            ITEM,
            DESCRICAO,
...
async function loadInspecao(id) {
    const schema = await getMolaSchema();
    const row = await queryOne(`SELECT ID,
    ...
    return {
        ...
        statusMaquina: row.STATUS_MAQUINA,
        alturaTipo: row.ALTURA_TIPO ?? "",
...
export async function listInspecoesMola() {
    if (!isOracleEnabled()) {
        return db.inspecoesMola.map((item) => mapFallbackInspecao(item));
    }
    const schema = await getMolaSchema();
    const rows = await queryRows(`SELECT ID
       FROM INS_MOLA_INSPECAO
...
export async function getInspecaoMolaById(id) {
    if (!isOracleEnabled()) {
        const found = db.inspecoesMola.find((item) => item.id === id);
        return found ? mapFallbackInspecao(found) : null;
    }
    return loadInspecao(id);
}
export async function createInspecaoMola(data) {
    const id = data.id ?? uid("MOLA");
    if (!isOracleEnabled()) {
        const payload = mapFallbackInspecao({ ...data, id });
        const idx = db.inspecoesMola.findIndex((item) => item.id === id);
        if (idx >= 0)
            db.inspecoesMola[idx] = payload;
        else
            db.inspecoesMola.push(payload);
        return payload;
    }
    const schema = await getMolaSchema();
    const maquina = await queryOne(`SELECT ID, CODIGO FROM INS_MOLA_MAQUINA WHERE CODIGO = :codigo AND ATIVO = 1`, { codigo: data.maquina });
    if (!maquina) {
    ...
    const existsRow = await queryOne(`SELECT COUNT(*) AS CNT FROM INS_MOLA_INSPECAO WHERE ID = :id`, { id });
    ...
    if (exists) {
        ...
        await execDml(`UPDATE INS_MOLA_INSPECAO
          SET ${updateClauses.join(", ")}
        WHERE ID = :id`, updateBinds);
    }
    else {
        ...
        await execDml(`INSERT INTO INS_MOLA_INSPECAO (${columns.join(", ")})
       VALUES (${values.join(", ")})`, binds);
    }
    ...
    for (const [idx, medicao] of medicoes.entries()) {
        ...
        await execDml(`INSERT INTO INS_MOLA_INSPECAO_AMOSTRA (${columns.join(", ")})
       VALUES (${values.join(", ")})`, binds);
    }
...
export async function listMaquinasMola() {
    if (!isOracleEnabled()) {
        return [
    ...
    const rows = await queryRows(`SELECT ID, CODIGO, DESCRICAO, ATIVO
