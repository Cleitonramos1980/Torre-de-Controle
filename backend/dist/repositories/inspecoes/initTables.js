...
async function columnExists(tableName, columnName) {
    ...
    try {
        const result = (await executeOracle(`SELECT * FROM ${normalizedTable} WHERE 1 = 0`));
        const metaData = Array.isArray(result?.metaData) ? result.metaData : [];
        return metaData.some((item) => String(item?.name ?? "").toUpperCase() === normalizedColumn);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (message.includes("ORA-00942"))
...
async function addColumnIfMissing(def) {
    if (!(await tableExists(def.table)))
        return;
    if (await columnExists(def.table, def.column))
        return;
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (message.includes("ORA-01430") ||
            message.includes("ORA-01031") ||
            message.includes("ORA-39726") ||
            message.toLowerCase().includes("unsupported add/drop column operation on compressed tables"))
            return;
        throw error;
    }
}
...
async function runBackfillMigrations() {
...
export async function ensureInspecoesTables() {
