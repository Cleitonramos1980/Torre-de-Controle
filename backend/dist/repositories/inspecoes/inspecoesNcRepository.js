...
export async function listTiposNc() {
    if (!isOracleEnabled()) {
        return db.inspecoesTiposNc.map((item) => ({
            id: item.id,
            setor: item.setor,
            nome: item.nome,
...
export async function createTipoNc(data) {
    ...
    if (!isOracleEnabled()) {
        ...
        db.inspecoesTiposNc.push(payload);
        return payload;
    }
    ...
    await execDml(`INSERT INTO INS_TIPO_NC (${columns.join(", ")})
...
export async function updateTipoNc(id, data) {
    if (!isOracleEnabled()) {
        const idx = db.inspecoesTiposNc.findIndex((item) => item.id === id);
        if (idx < 0)
            return null;
        const merged = {
    ...
    const schema = await getNcSchema();
    const current = await queryOne(`SELECT ID,
            SETOR_ID,
            NOME,
            CATEGORIA,
    ...
    await execDml(`UPDATE INS_TIPO_NC
    ...
    const setorNomeRow = await queryOne(`SELECT NOME FROM INS_SETOR WHERE ID = :id`, { id: setorId });
    return {
        id,
