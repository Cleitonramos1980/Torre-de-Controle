...
function toEstoqueView(materiais, estoque) {
    return materiais.map((m) => {
        const get = (planta) => estoque.find((e) => e.codmat === m.codmat && e.planta === planta)?.qtdDisponivel ?? 0;
        return {
...
export async function assistenciaRoutes(app) {
    ...
    app.post("/api/assistencia/os", async (req) => {
        ...
        return assistRepo.createOS(body);
    });
    app.put("/api/assistencia/os/:id/status", async (req, reply) => {
        ...
        if (!os)
            return reply.status(404).send({ error: { message: "OS não encontrada" } });
        return os;
    });
    app.post("/api/assistencia/os/:id/transition", async (req, reply) => {
        ...
        if (!t)
            return reply.status(400).send({ error: { message: "Evento inválido" } });
    ...
    app.post("/api/assistencia/requisicoes", async (req) => {
        ...
        return assistRepo.createReq(body);
    });
    app.put("/api/assistencia/requisicoes/:id/status", async (req) => {
        ...
        return assistRepo.updateReqStatus(id, body);
    });
    ...
    app.post("/api/assistencia/consumos", async (req) => {
        ...
        return assistRepo.createConsumo(body);
    });
    ...
    app.get("/api/os-transition-log", async (req) => {
        ...
        if (q.osId)
            return assistRepo.listTransitionByOS(q.osId);
