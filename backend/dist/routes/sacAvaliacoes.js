...
function buildAvaliacaoLink(token) {
    return `/avaliacao?token=${token}`;
}
export async function sacAvaliacoesRoutes(app) {
    // GET /api/sac/avaliacoes — list with optional filters
    app.get("/api/sac/avaliacoes", async (req) => {
        const q = req.query;
        let list = db.sacAvaliacoes;
        if (q.atendimentoId)
            list = list.filter((a) => a.atendimentoId === q.atendimentoId);
        if (q.statusEnvio)
            list = list.filter((a) => a.statusEnvio === q.statusEnvio);
        if (q.statusResposta)
            list = list.filter((a) => a.statusResposta === q.statusResposta);
        if (q.planta)
            list = list.filter((a) => a.planta === q.planta);
        return list;
    });
    ...
    app.get("/api/sac/avaliacoes/public", async (req, reply) => {
        ...
        if (!avaliacao)
            return reply.status(404).send({ error: { message: "Avaliação não encontrada." } });
        return {
            id: avaliacao.id,
    ...
    app.post("/api/sac/atendimentos/:id/avaliacao/link", async (req, reply) => {
        ...
        if (!atendimento)
            return reply.status(404).send({ error: { message: "Atendimento não encontrado." } });
        ...
        const avaliacao = {
            ...
            responsavelAtendimento: req.authUser?.nome,
            encerradoAt: atendimento.encerradoAt,
            token,
            link: buildAvaliacaoLink(token),
            canalEnvio: "WHATSAPP",
            statusEnvio: "NAO_ENVIADA",
            statusResposta: "NAO_RESPONDIDA",
            envioLogs: [],
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
        };
        db.sacAvaliacoes.push(avaliacao);
        appendAudit("GERAR_LINK", "SAC_AVALIACAO", avaliacao.id, `Link gerado para atendimento ${id}`, req.authUser?.nome ?? "system");
        return avaliacao;
    });
    ...
    app.post("/api/sac/atendimentos/:id/avaliacao/enviar", async (req, reply) => {
        ...
        if (idx < 0) {
            ...
            db.sacAvaliacoes.push(avaliacao);
            appendAudit("ENVIAR", "SAC_AVALIACAO", avaliacao.id, `Pesquisa enviada para ${atendimento.telefone}`, req.authUser?.nome ?? "system");
            return avaliacao;
        }
        const avaliacao = db.sacAvaliacoes[idx];
        ...
        appendAudit("ENVIAR", "SAC_AVALIACAO", avaliacao.id, `Pesquisa enviada/reenviada`, req.authUser?.nome ?? "system");
        return db.sacAvaliacoes[idx];
    });
    ...
    app.post("/api/sac/avaliacoes/:id/reenviar", async (req, reply) => {
        ...
        appendAudit("REENVIAR", "SAC_AVALIACAO", avaliacao.id, `Pesquisa reenviada`, req.authUser?.nome ?? "system");
        return db.sacAvaliacoes[idx];
    });
    ...
    app.get("/api/sac/historico-cliente", async (req) => {
        ...
        const matchAtend = (a) => {
            if (codcli && a.codcli === codcli)
                return true;
            if (cgcent && a.cgcent === cgcent)
                return true;
            if (cliente && a.clienteNome.toLowerCase().includes(cliente))
                return true;
