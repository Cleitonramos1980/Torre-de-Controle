import { z } from "zod";
import { mockAssistenciasTerceirizadas, mockItensEmAssistencia, mockMovimentacoes, } from "../repositories/seedAssistenciaTerceirizadaData.js";
...
export async function assistenciaTerceirizadaRoutes(app) {
    // Dashboard
    ...
    app.post("/api/assistencia-terceirizada", async (req) => {
        const body = req.body;
        const novo = { ...body, id: `AT-${String(nextATId++).padStart(3, "0")}`, criadoAt: new Date().toISOString().slice(0, 10), atualizadoAt: new Date().toISOString().slice(0, 10) };
    ...
    // Movimentações
    app.get("/api/assistencia-terceirizada/movimentacoes", async () => [...movimentacoes].sort((a, b) => new Date(b.dataHora).getTime() - new Date(a.dataHora).getTime()));
