...
import { inventarioInternoRepository } from "../repositories/inventario/inventarioInternoRepository.js";
...
const inventarioInternoCreateSchema = z.object({
    id: z.string().optional(),
    numero: z.string().optional(),
    lojaId: z.string().min(1),
    filialWinthor: z.string().optional(),
    codlocalWinthor: z.string().optional(),
    origem: z.string().optional(),
    observacoes: z.string().optional(),
    itens: z.array(z.object({
        id: z.string().optional(),
        codprod: z.union([z.number(), z.string()]).optional(),
        codigoItem: z.string().optional(),
        descricao: z.string().min(1),
        codlocalWinthor: z.string().optional(),
        qt1: z.union([z.number(), z.string()]).optional(),
        qt2: z.union([z.number(), z.string()]).optional(),
        qt3: z.union([z.number(), z.string()]).optional(),
        qtestger: z.union([z.number(), z.string()]).optional(),
    })).optional(),
});
const inventarioInternoAddItemsSchema = z.object({
    itens: z.array(z.object({
        id: z.string().optional(),
        codprod: z.union([z.number(), z.string()]).optional(),
        codigoItem: z.string().optional(),
...
const inventarioInternoContagemSchema = z.object({
    qt1: z.union([z.number(), z.string()]).optional(),
    qt2: z.union([z.number(), z.string()]).optional(),
    qt3: z.union([z.number(), z.string()]).optional(),
    qtestger: z.union([z.number(), z.string()]).optional(),
    ...
    observacao: z.string().optional(),
    responsavelContagem: z.string().optional(),
});
const inventarioFilialMapSchema = z.object({
    lojaId: z.string().min(1),
    filialWinthor: z.string().optional(),
    codlocalWinthor: z.string().optional(),
...
const inventarioInternoListSchema = z.object({
    status: z.string().optional(),
    lojaId: z.string().optional(),
});
const inventarioExportQuerySchema = z.object({
    onlyErrors: z.coerce.boolean().optional(),
});
const inventarioExportModeSchema = z.object({
    mode: z.enum(["dry-run", "real"]).optional(),
});
export async function inventarioRoutes(app) {
    ...
    app.get("/api/inventario/contagens/:id", async (req, reply) => {
        try {
            ...
            if (!contagem)
                return reply.status(404).send({ error: { message: "Contagem nao encontrada." } });
            return contagem;
        }
        catch (error) {
            return sendError(reply, error);
    ...
    app.get("/api/inventario/interno/filiais-mapeamento", async (_req, reply) => {
        try {
            return await inventarioInternoRepository.listFilialMappings();
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.put("/api/inventario/interno/filiais-mapeamento", async (req, reply) => {
        try {
            const body = inventarioFilialMapSchema.parse(req.body);
            return await inventarioInternoRepository.upsertFilialMapping(body, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.post("/api/inventario/interno", async (req, reply) => {
        try {
            const body = inventarioInternoCreateSchema.parse(req.body);
            return await inventarioInternoRepository.createInventarioInterno(body, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.get("/api/inventario/interno", async (req, reply) => {
        try {
            const query = inventarioInternoListSchema.parse(req.query);
            return await inventarioInternoRepository.listInventariosInternos(query);
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.get("/api/inventario/interno/:id", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            const data = await inventarioInternoRepository.getInventarioInternoById(id);
            if (!data)
                return reply.status(404).send({ error: { message: "Inventario interno nao encontrado." } });
            return data;
        }
        catch (error) {
            return sendError(reply, error);
    ...
    app.post("/api/inventario/interno/:id/itens", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            const body = inventarioInternoAddItemsSchema.parse(req.body);
            return await inventarioInternoRepository.addItensInventarioInterno(id, body, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.put("/api/inventario/interno/:id/itens/:itemId/contagem", async (req, reply) => {
        try {
            const { id, itemId } = z.object({ id: z.string(), itemId: z.string() }).parse(req.params);
            const body = inventarioInternoContagemSchema.parse(req.body);
            return await inventarioInternoRepository.registrarContagemItem(id, itemId, body, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.put("/api/inventario/interno/:id/validar", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            return await inventarioInternoRepository.validarInventarioInterno(id, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.put("/api/inventario/interno/:id/solicitar-finalizacao", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            return await inventarioInternoRepository.solicitarFinalizacaoInventario(id, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.put("/api/inventario/interno/:id/finalizar", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            return await inventarioInternoRepository.finalizarInventario(id, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.put("/api/inventario/interno/:id/aprovar-finalizacao", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            return await inventarioInternoRepository.aprovarFinalizacaoInventario(id, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.post("/api/inventario/interno/:id/exportacao/dry-run", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            const query = inventarioExportQuerySchema.parse(req.query);
            return await inventarioInternoRepository.dryRunExportacaoWinthor(id, actorFromReq(req), query);
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.post("/api/inventario/interno/:id/exportacao", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            const body = inventarioExportModeSchema.parse(req.body ?? {});
            return await inventarioInternoRepository.exportarParaWinthor(id, actorFromReq(req), body);
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.get("/api/inventario/interno/:id/exportacao", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            return await inventarioInternoRepository.getExportacaoInventario(id);
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.post("/api/inventario/interno/:id/exportacao/reprocessar", async (req, reply) => {
        try {
            const { id } = z.object({ id: z.string() }).parse(req.params);
            return await inventarioInternoRepository.reprocessarExportacaoComErro(id, actorFromReq(req));
        }
        catch (error) {
            return sendError(reply, error);
        }
    ...
    app.put("/api/inventario/checklists/:id", async (req, reply) => {
        try {
            ...
            if (!updated)
                return reply.status(404).send({ error: { message: "Checklist nao encontrado." } });
            return updated;
        }
        catch (error) {
            return sendError(reply, error);
    ...
    app.put("/api/inventario/checklists/:checklistId/itens/:itemId", async (req, reply) => {
        try {
            ...
            if (!updated)
                return reply.status(404).send({ error: { message: "Item de checklist nao encontrado." } });
            return updated;
        }
        catch (error) {
            return sendError(reply, error);
